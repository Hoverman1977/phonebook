...
if (isset($_POST['action'])){
	if ($_POST['action'] == 'upload') {
		...
		if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
			$message = $wan->process_file($target_path);
		} else {
...
if (isset($_GET['month']) && $_GET['month'] != "" && (!isset($_GET['action']))){
	$month = $_GET['month'];
	$year = $_GET['year'];	
	$bill_info = $wan->get_bills_by_month($month, $year);
...
?>
                        ...
                        <?php
                                if (isset($bill_info)){
					$month_info = $wan->get_bill_months();
					$count = mysql_num_rows($bill_info);
					$year_info = $wan->get_bill_years();
	                                print ('<form method="get" action="" id="srchform" onsubmit="return false;"><div class="form">');
    	                            $js = "submitform('srchform');";
				    $dlbizlink_js = "window.open('dl.php?module=wan&type=bizlink_wan&month=".$month."&year=".$year."');";
